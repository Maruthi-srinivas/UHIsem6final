...
//       const response = await fetch(`http://192.168.25.159:5500/verifyOTP`, {
//         method: 'POST',
...
//       const userFind= await fetch(`http://192.168.25.159:5501/user/find`,{
//         method:'POST',
//         headers:{'Content-Type':'application/json'},
...
const handleVerifyOtp = async () => {
  ...
  try {
    ...
    if (otpResponse.ok) {
      ...
      if (isDoctorLogin) {
        // Fetch doctor details from Shanmuga backend
        const doctorResponse = await fetch(`http://192.168.25.159:5501/doctor/getById`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          // Assuming phoneNumber (without +91) is used to fetch the doctor detail
          body: JSON.stringify({ id: phoneNumber.slice(3) }),
        });
            ...
            routes: [
              {
                ...
                state: {
                  routes: [
                    {
                      name: "Home",
                      params: { DoctorId: doctorResult.id },
                    },
                  ],
                },
              },
        ...
        if (userResult.exists) {
          ...
          // For new users, navigate to RegForm
